...
class Phenny(irc.Bot): 
   ...
   def call(self, func, origin, bitbot, input): 
      try: func(bitbot, input)
      except Exception, e: 
         self.error(origin)
         ...
         for regexp, funcs in items: 
            for func in funcs: 
               ...
               if match: 
                  ...
                  bitbot = self.wrapped(origin, text, match)
                  input = self.input(origin, text, bytes, match, event, args)

                  if func.thread: 
                     targs = (func, origin, bitbot, input)
                     t = threading.Thread(target=self.call, args=targs)
                     t.start()
                  else: self.call(func, origin, bitbot, input)
