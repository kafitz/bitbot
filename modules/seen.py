...
def seen(bitbot, input): 
   """.seen <nick> - Reports when <nick> was last seen."""
   nick = input.group(2)
   if not nick:
      return bitbot.reply("Need a nickname to search for...")
   nick = nick.lower()

   if not hasattr(bitbot, 'seen'): 
      return bitbot.reply("?")

   if bitbot.seen.has_key(nick): 
      channel, t = bitbot.seen[nick]
      t = time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime(t))

      msg = "I last saw %s at %s on %s" % (nick, t, channel)
      bitbot.reply(msg)
   else: bitbot.reply("Sorry, I haven't seen %s around." % nick)
seen.rule = (['seen'], r'(\S+)')
